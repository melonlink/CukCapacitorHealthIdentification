...
P0Sup = diag([cfg.ltvP0Alpha, (cfg.ltvP0RScale * obs.Rtruth(1))^2]);
supFast = [0; 0]; supSlow = [0; 0];
supCount = [0; 0]; supHold = [0; 0];
...
for k = 1:n
    ...
    switch method
        ...
        case "M4 TS-SRKE"
            % M2 kernel plus the two-time-scale mean-shift supervisor of
            % Paper Verification v1.3 (fast/slow EWMA difference on the
            % clipped normalized innovations of accepted AND rejected
            % rows; alarm resets that direction's covariance).
            P = P + cfg.ltvQ * cycleCount;
            if obs.validC(k)
                residualC(k) = obs.zC(k) - obs.hC(k) * theta(1);
                [theta, P, nisC(k), acceptedC(k), epsC] = ...
                    supervisedMeasurement(theta, P, obs.zC(k), ...
                    [obs.hC(k), 0], obs.RC(k), cfg.nisGate);
                informationC(k) = obs.hC(k)^2 / obs.RC(k);
                [supFast, supSlow, supCount, supHold, P] = ...
                    superviseDirection(1, epsC, supFast, supSlow, ...
                    supCount, supHold, P, P0Sup, cfg);
            end
            if obs.validR(k)
                residualR(k) = obs.zR(k) - obs.hR(k) * theta(2);
                [theta, P, nisR(k), acceptedR(k), epsR] = ...
                    supervisedMeasurement(theta, P, obs.zR(k), ...
                    [0, obs.hR(k)], obs.RR(k), cfg.nisGate);
                informationR(k) = obs.hR(k)^2 / obs.RR(k);
                [supFast, supSlow, supCount, supHold, P] = ...
                    superviseDirection(2, epsR, supFast, supSlow, ...
                    supCount, supHold, P, P0Sup, cfg);
            end
        case "M3 Dual EKF"
...
function [theta, P, nis, accepted, epsNorm] = supervisedMeasurement( ...
        theta, P, z, H, R, gate)
innovation = z - H * theta;
S = H * P * H.' + R;
nis = innovation^2 / max(S, eps);
epsNorm = innovation / sqrt(max(S, eps));
accepted = isfinite(nis) && nis <= gate;
if accepted
    K = P * H.' / S;
    theta = theta + K * innovation;
...
function [mF, mS, cnt, hold, P] = superviseDirection(d, epsNorm, ...
        mF, mS, cnt, hold, P, P0, cfg)
epsNorm = max(min(epsNorm, cfg.supClip), -cfg.supClip);
cnt(d) = cnt(d) + 1;
if cnt(d) <= cfg.supWarmupRows
    mF(d) = mF(d) + (epsNorm - mF(d)) / cnt(d);
    mS(d) = mF(d);
    return;
end
mF(d) = mF(d) + cfg.supFastRate * (epsNorm - mF(d));
mS(d) = mS(d) + cfg.supSlowRate * (epsNorm - mS(d));
if hold(d) > 0
    hold(d) = hold(d) - 1;
elseif abs(mF(d) - mS(d)) > cfg.supThreshold
    P(d, d) = P0(d, d);
    P(1, 2) = 0; P(2, 1) = 0;
    mF(d) = mS(d);
    hold(d) = cfg.supHoldoffRows;
end
end

function summary = paper_algorithm_selection_engine(packageRoot)
%PAPER_ALGORITHM_SELECTION_ENGINE Frozen O1 estimator selection campaign.
...
save(fullfile(cfg.rawDir, "paper_algorithm_selection_v1_workspace.mat"), ...
...
cfg.frozenRowsPath = fullfile(repoRoot, "paper_verification_v11", ...
    "results", "raw", "factorial_rows.csv");
cfg.frozenDynamicPath = fullfile(repoRoot, "paper_verification_v11", ...
...
assert(height(rows) == 2304, "algsel:StaticCount", ...
    "Expected 2304 frozen O1 rows, found %d.", height(rows));
...
for iMethod = 1:numel(cfg.methods)
    ...
    for iMode = 1:numel(cfg.modes)
        ...
        item.source_model = "FROZEN_PV11_MODEL_B_ANCHOR";
        ...
        for k = 1:height(subset)
            ...
            tr.source_model = "FROZEN_PV11_MODEL_B_ANCHOR";
            ...
            tr.notes = "Frozen v1.1 O1 blind row; identical case/seed.";
...
method = cfg.methods.';
state_dimension = [2; 3; 3];
scalar_divisions_per_observation = [1; 1; 2];
multiplications_per_observation = [28; 46; 62];
additions_per_observation = [20; 34; 47];
memory_scalars = [22; 31; 40];
estimated_execution_us_per_observation = [1.36; 2.12; 3.05];
accepted_observations_per_frame = repmat(cfg.healthFrameCycles, 3, 1);
...
health_frame_ms = repmat(1e3 * cfg.healthReportPeriodS, 3, 1);
source = repmat("Frozen Paper Verification v1 operation-count model", 3, 1);
...
for method = ["M2 TS-SLTVKE", "M3 Dual EKF"]
...
if rlsDominatesCore && tsMaterialRobustness
...
lockTable = table(cfg.methods(:), [cfg.rlsLambda; NaN; NaN], ...
    [cfg.rlsP0; NaN; NaN], ...
    [NaN; cfg.ltvQ(1, 1); cfg.dualQ(1, 1)], ...
    [NaN; cfg.ltvQ(2, 2); cfg.dualQ(2, 2)], ...
    [NaN; cfg.ltvP0Alpha; cfg.dualP0Alpha], ...
    [NaN; cfg.ltvP0RScale; cfg.dualP0RScale], ...
    [NaN; cfg.nisGate; Inf], ...
    repmat(string(mat2str(cfg.CBounds / cfg.Cb)), 3, 1), ...
    repmat(string(mat2str(cfg.RBounds / cfg.Rb)), 3, 1), ...
    ["Sequential scalar direction-specific RLS"; ...
    "Frozen sequential Joseph KF"; "Frozen dual state/parameter EKF"], ...
    repmat(cfg.algorithmSha256, 3, 1), ...
...
protocol = sprintf(strjoin(["# Algorithm-selection protocol\n\n", ...
    ...
    "- Static: inherited 48 cases x 4 noise x 4 skew from v1.1.\n", ...
    ...
    "- No adaptive Q, change detector, estimator retuning, or weighted score.\n"], ""), ...
...
manifest = table(["static"; "ramp_long"; "ramp_0p1_crosscheck"; ...
    ...
    ["FROZEN_PV11_MODEL_B_ANCHOR"; "TRACE_DERIVED_OBSERVATION"; ...
    "FULL_SWITCHING_MODEL_A_EQUATIONS"; ...
    "FULL_SWITCHING_MODEL_A_EQUATIONS"; ...
    "FULL_SWITCHING_MODEL_A_EQUATIONS"], ...
    ["v1.1 raw paired rows"; "frozen O1 grouped sufficient statistics"; ...
...
decisionReport = sprintf(strjoin(["# Final estimator decision\n\n", ...
    "1. **Does O1-RLS retain the v1.1 static advantage?** Yes for ESR, ", ...
...
readme = sprintf(strjoin(["# Paper Algorithm Selection v1\n\n", ...
    "Final decision: **%s**; primary manuscript realization: **%s**.\n\n", ...
    "Run `scripts/run_paper_algorithm_selection_v1.m`, then ", ...
    "`scripts/validate_paper_algorithm_selection_v1.m`. Results are split ", ...
    "into ten tables, twelve figures, raw trace/history, four paper reports, ", ...
    "and a GPT review package. Frozen Paper Verification v1.1 files are ", ...

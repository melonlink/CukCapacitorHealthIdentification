...
On the frozen 48 x 4 x 4 O1 blind matrix, TS-D-RLS achieved C/ESR mean MAPE 0.3727%/0.2222% and 13.19-cycle convergence. TS-SLTVKE achieved 0.3011%/0.6428% and 33.04 cycles; Dual EKF achieved 0.3043%/1.1923% and 40.57 cycles. Native and Equal-Report use identical estimates; Equal-Report changes only the visible 20.48 ms cadence.
...
The frozen 257-post-cycle TS-SLTVKE failures were retained: C-step error 19.2911% and ESR-step error 49.9379%. These discontinuities are stress tests, not claimed physical aging rates.

## 4. Slow degradation tracking

- 0.1 s: RLS combined nRMSE 0.1161, TS-SLTVKE 0.2159; normalized lag 8.053% versus 22.027%; dominance result: TS-D-RLS.
- 1.0 s: RLS combined nRMSE 0.0164, TS-SLTVKE 0.0453; normalized lag 1.360% versus 3.537%; dominance result: TS-D-RLS.
- 10.0 s: RLS combined nRMSE 0.0159, TS-SLTVKE 0.0453; normalized lag 1.752% versus 4.492%; dominance result: TS-D-RLS.
- 100.0 s: RLS combined nRMSE 0.0213, TS-SLTVKE 0.0476; normalized lag 2.827% versus 4.868%; dominance result: TS-D-RLS.
...
**DUAL_REALIZATION**. The observation framework supports two realizations: TS-D-RLS is the primary low-cost identifier, while TS-SLTVKE provides the calibrated-confidence extension. The decision uses scientific dominance and Pareto evidence, not an arbitrary weighted score.

Paired bootstrap used 10000 resamples; 7 metric/mode rows were retained.

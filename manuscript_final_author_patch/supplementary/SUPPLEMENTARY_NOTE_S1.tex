...
The selected E1 rising-edge policy uses a 0.5-$\mu$s guard and three points on each side of the reconstructed edge. Three start timestamps span 1.830~$\mu$s, while their complete apertures span 2.150~$\mu$s. Consequently, a 2.0-$\mu$s point-timestamp-only design is a false pass. The locked ESR edge-fit window is $T_{w,R}=2.2~\mu$s and retains 50~ns full-aperture margin. It is distinct from the $T_{w,C}=2.0~\mu$s charge-safe window used by the capacitance observation and PE proof. Ten conversions per ADC per switching cycle correspond to 45.75\% utilization. Four DMA streams carry 4.0~MB/s in aggregate.
